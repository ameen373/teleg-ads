/**
 * Telega.ads - Ultra-Enterprise Server Core (V6 Architecture)
 * Unified Modular System with Integrated Anti-Fraud, Redis Caching, and Financial Engine
 */
...
// --- استيراد النماذج الأساسية مع نماذج النظام الحسابي والإعلاني ---
const models = require('./models');
const { User, Wallet, Transaction, Link, Campaign } = models;

// تعريف النماذج المكملة تلقائياً في حال عدم تصديرها من models.js لضمان الاستقرار المطلق
...
// =========================================================================
// --- 1. إعدادات الأمان والتسجيل الشامل (Security & Logging Core) ---
// =========================================================================
...
// فرض رأس الاستجابة الحميّة وتحديد الكاش للـ APIs
...
// محرك السجلات الاحترافي (Winston Engine)
...
// =========================================================================
// --- 2. الثوابت وإعداد اتصال Redis وقاعدة البيانات ---
// =========================================================================
...
// إعداد كاش Redis مع آلية التعافي التلقائي (Fault-Tolerant Cache)
...
redis.on('error', (err) => { redisConnected = false; logger.error('⚠️ Redis connection error: ' + err.message); });
redis.on('ready', () => { redisConnected = true; console.log('✅ Redis Client Ready & Active'); });
...
// الاتصال المباشر بقاعدة البيانات Mongo
mongoose.connect(CONFIG.MONGO_URI, {
  maxPoolSize: 50,
  minPoolSize: 10,
  serverSelectionTimeoutMS: 5000,
  socketTimeoutMS: 45000
}).then(() => console.log('✅ Enterprise MongoDB Connection Established'))
  .catch(err => { logger.error('❌ Critical Mongo Connection Error:', err); process.exit(1); });

// =========================================================================
// --- 3. المساعدات والحماية المتقدمة (Anti-Fraud & Helpers) ---
// =========================================================================
...
// تحديد معدل الطلبات وحجب البوتات
const globalLimiter = rateLimit({
  windowMs: 15 * 60 * 1000,
  max: 200,
  message: { success: false, message: 'تم تجاوز حد الطلبات المسموح به' }
});
...
// =========================================================================
// --- 4. وسيط المصادقة المزدوجة (Hybrid Authentication Middleware) ---
// =========================================================================
const authenticateUser = async (req, res, next) => {
  try {
    let user = null;

    // 1. التحقق عبر توكن Bearer JWT
    ...
    // 2. التحقق المباشر عبر Telegram InitData
    if (!user) {
      ...
      if (tgUser) {
        ...
        if (!user) {
          ...
          await Wallet.create({ userId: newUser._id, balance: 0, pendingBalance: 0 });
    ...
    if (!user) {
      return res.status(401).json({ success: false, message: 'مصادقة غير صالحة، يرجى فتح التطبيق من تليجرام' });
    }

    if (user.isBanned) {
      return res.status(403).json({ success: false, message: 'حسابك معطل بسبب مخالفة معايير الاستخدام' });
    }
    ...
    return res.status(500).json({ success: false, message: 'خطأ داخلي في نظام المصادقة', error: error.message });
  }
};

const requireAdmin = (req, res, next) => {
  if (!req.user || String(req.user.telegramId).trim() !== CONFIG.ADMIN_ID) {
    return res.status(403).json({ success: false, message: 'غير مصرح: صلاحديد الإدارة مطلوبة' });
  }
  next();
};

// =========================================================================
// --- 5. مسارات تسجيل الدخول وتوثيق الحساب (/api/auth) ---
// =========================================================================
app.post('/api/auth/login', async (req, res) => {
  try {
    ...
    if (!tgId) return res.status(401).json({ success: false, message: 'بيانات الاعتماد غير صالحة' });
    ...
    if (!user) {
      ...
      await Wallet.create({ userId: user._id, balance: 0, pendingBalance: 0 });
    }
    ...
    res.json({
      ...
      botUsername: '@' + CONFIG.OFFICIAL_BOT_URL.split('/').pop()
    });
  } catch (e) {
    res.status(500).json({ success: false, message: 'فشل في تسجيل الدخول', error: e.message });
  }
});

// =========================================================================
// --- 6. النظام المالي الذري للمحفظة (Atomic Wallet Engine) ---
// =========================================================================
app.get('/api/wallet', authenticateUser, async (req, res) => {
  try {
    let wallet = await Wallet.findOne({ userId: req.userId });
    if (!wallet) {
      wallet = await Wallet.create({ userId: req.userId, balance: 0, pendingBalance: 0 });
    }
    const transactions = await Transaction.find({ userId: req.userId }).sort({ createdAt: -1 }).limit(20).lean();
...
app.post('/api/wallet/deposit', authenticateUser, async (req, res) => {
  ...
  if (isNaN(numAmount) || numAmount <= 0) {
    return res.status(400).json({ success: false, message: 'المبلغ المحدد غير صالح' });
  }

  const cleanTxid = String(txid || '').trim();
  if (cleanTxid.length < 8) {
    return res.status(400).json({ success: false, message: 'يرجى إدخال هاش المعاملة الصحيح (TxID)' });
  }
  ...
  try {
    await session.withTransaction(async () => {
      ...
      if (existingTx) throw new Error('تم استخدام رقم هذه المعاملة (TxID) مسبقاً');
      ...
      await Transaction.create([{
        ...
        amount: numAmount,
        status: 'pending',
        description: `طلب إيداع عبر شبكة ${network || 'USDT'}`,
    ...
    res.json({ success: true, message: 'تم تقديم طلب الإيداع وهو قيد المراجعة حالياً' });
  } catch (error) {
...
app.post('/api/wallet/withdraw', authenticateUser, async (req, res) => {
  ...
  if (isNaN(numAmount) || numAmount < 10) {
    return res.status(400).json({ success: false, message: 'الحد الأدنى للسحب هو 10$ (أو 1000 سنت)' });
  }
  ...
  try {
    await session.withTransaction(async () => {
      // الخصم الذري لضمان عدم حدوث رصيد بالسالب مع السباق الموازي (Race Conditions)
      ...
      if (!wallet) throw new Error('رصيد المحفظة المتاح غير كافٍ لإتمام عملية السحب');
      ...
      await Transaction.create([{
        ...
        amount: numAmount,
        status: 'pending',
        description: `طلب سحب إلى المحفظة: ${walletAddress} (${network || 'TRC20'})`
      }], { session });
    });
...
// =========================================================================
// --- 7. محرك الروابط المختصرة التكيفي (Smart Link Shortener Engine) ---
// =========================================================================
app.post('/api/links', authenticateUser, async (req, res) => {
  try {
    ...
    if (!cleanUrl || !validUrl.isWebUri(cleanUrl)) {
      return res.status(400).json({ success: false, message: 'الرابط المرفق غير صالح' });
    }

    if (isPhishingOrMalicious(cleanUrl)) {
      return res.status(400).json({ success: false, message: 'الرابط ينتهك سياسات الأمان والموثوقية' });
    }

    if (cleanUrl.includes(CONFIG.APP_DOMAIN)) {
      return res.status(400).json({ success: false, message: 'لا يمكن اختصار روابط الدومين الحالي' });
    }
    ...
    res.json({ success: true, message: 'تم إنشاء الرابط بنجاح', link: newLink, shortUrl: shortFullUrl });
  } catch (error) {
    if (error.code === 11000) {
      return res.status(400).json({ success: false, message: 'الرمز المختصر مستخدم بالفعل' });
    }
    res.status(500).json({ success: false, message: 'فشل في إنشاء الرابط', error: error.message });
  }
});
...
// =========================================================================
// --- 8. محرك الإعلانات والجسر والتحقق من المشاهدات (Ad & Impression Bridge) ---
// =========================================================================
app.post('/api/campaigns', authenticateUser, async (req, res) => {
  ...
  if (!title || !numBudget || numBudget < 5 || !targetUrl) {
    return res.status(400).json({ success: false, message: 'جميع البيانات مطلوبة والحد الأدنى للميزانية هو 5$' });
  }
  ...
  try {
    await session.withTransaction(async () => {
      ...
      if (!wallet) throw new Error('رصيد المحفظة غير كافٍ لإطلاق هذه الحملة');
...
app.post('/api/init-click', validateTraffic, async (req, res) => {
  try {
    ...
    // جلب حملة إعلانية داخلية نشطة لا تعود لنفس صاحب الرابط
    ...
    res.status(500).json({ success: false, message: 'خطأ في تهيئة جلسة التوجيه', error: error.message });
  }
});
...
app.post('/api/impression', validateTraffic, clickLimiter, async (req, res) => {
  ...
  try {
    ...
    if (!sessionId || !bridgeToken) {
      ...
      return res.status(400).json({ success: false, message: 'رمز الجلسة مفقود' });
    }

    const cachedToken = await safeRedisGet(`bridge:token:${sessionId}`);
    if (cachedToken && cachedToken !== bridgeToken) {
      await sessionDb.abortTransaction();
      return res.status(403).json({ success: false, message: 'تم كشف محاولة اختراق أو تزوير للجلسة' });
    }

    const clickSession = await ClickSession.findById(sessionId).session(sessionDb);
    if (!clickSession || clickSession.ip !== req.ip) {
      await sessionDb.abortTransaction();
      return res.status(403).json({ success: false, message: 'الجلسة غير صالحة أو منتهية' });
    }

    const dwellTime = Date.now() - new Date(clickSession.createdAt).getTime();
    if (dwellTime < 4500 && (Number(duration) || 0) < 5) {
      await sessionDb.abortTransaction();
      return res.status(400).json({ success: false, message: 'لم يتم استيفاء وقت المكوث المطلوب (5 ثوانٍ)' });
    }
    ...
    // احتساب الزيارة المكررة بدون إضافة أرباح لمنع الاحتيال
    if (isDuplicate) {
    ...
    const publisherEarning = 0.0012; // ربح الناشر لكل زيارة فريدة مؤكدة
    ...
    // خصم الميزانية من المعلن وتوجيه الرصيد المحجوز للناشر
    ...
    // إضافة الأرباح لرصيد الناشر المعلق لغاية التسوية التلقائية
    ...
    await sessionDb.abortTransaction();
    res.status(500).json({ success: false, message: 'خطأ أثناء تسجيل المشاهدة', error: error.message });
  } finally {
...
// =========================================================================
// --- 9. مسارات الإحصائيات والإدارة الشاملة (Admin & Stats) ---
// =========================================================================
...
app.post('/api/admin/transaction/action', authenticateUser, requireAdmin, async (req, res) => {
  ...
  try {
    await session.withTransaction(async () => {
      ...
      if (!tx || tx.status !== 'pending') throw new Error('المعاملة غير موجودة أو تم البت فيها مسبقاً');
    ...
    res.json({ success: true, message: `تمت المعالجة بنجاح: ${action}` });
  } catch (error) {
    res.status(400).json({ success: false, message: error.message });
  } finally {
    session.endSession();
...
// =========================================================================
// --- 10. المهمة المبرمجة لتسوية الأرباح المعلقة (Settlement Cron Engine) ---
// =========================================================================
  ...
  try {
    ...
    for (const hold of readyHolds) {
      ...
      try {
        ...
        logger.error(`Error Releasing Hold [${hold._id}]: ${err.message}`);
      } finally {
        session.endSession();
      }
    }
...
// =========================================================================
// --- 11. تسليم الصفحات والمعالجة العامة للأخطاء ---
// =========================================================================
...
app.use((err, req, res, next) => {
  ...
  res.status(err.status || 500).json({
    success: false,
    message: process.env.NODE_ENV === 'production' ? 'حدث خطأ داخلي في الخادم' : err.message
  });
});
...
app.listen(PORT, () => console.log(`🚀 Ultra-Enterprise Telega.ads Core Active on Port ${PORT}`));
