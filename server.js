...
// --- Body Parsing Middleware (JSON, Text for sendBeacon, URL-Encoded) ---
app.use(express.json({ limit: '10kb' }));
app.use(express.text({ limit: '10kb', type: ['text/plain', 'application/json'] }));
...
// Helper parse for sendBeacon body payloads (Text / JSON)
const parseBeaconPayload = (req) => {
...
// =========================================================================
// --- Instant Save & navigator.sendBeacon Routes (الحفظ الفوري في قاعدة البيانات) ---
// =========================================================================

// مسار احتياطي يستقبل طلبات navigator.sendBeacon العامة
app.post(['/api/beacon', '/api/save-instant'], authMiddleware, async (req, res) => {
  try {
    const payload = parseBeaconPayload(req);
    const { type, data } = payload;
    const userId = req.userId;

    if (!type || !data) {
      return res.status(200).json({ success: true, warning: 'بيانات غير كاملة ولكن تم إرجاع 200 لمنع التعليق' });
    }

    switch (type) {
      case 'link':
        if (data.targetUrl && validUrl.isWebUri(data.targetUrl)) {
          const shortCode = data.shortCode || crypto.randomBytes(3).toString('hex');
          await Link.create({
            userId: userId,
            publisherTelegramId: req.user.telegramId,
            telegramId: req.user.telegramId,
            title: data.title || 'رابط سريع',
      ...
      case 'ad':
        if (data.title && data.targetUrl && Number(data.totalBudget) >= 5) {
          const budget = Number(data.totalBudget);
          const userObj = await User.findOne({ _id: userId, availableBalance: { $gte: budget } });
          if (userObj) {
            userObj.availableBalance -= budget;
            await userObj.save();
            await Ad.create({
              userId,
              advertiserId: userId,
      ...
      case 'deposit':
        if (Number(data.amount) >= 1 && data.network && data.txid) {
      ...
      case 'withdraw':
        if (Number(data.amount) >= 30 && data.network && data.walletAddress) {
          const amt = Number(data.amount);
          const userObj = await User.findOne({ _id: userId, availableBalance: { $gte: amt } });
          if (userObj) {
            userObj.availableBalance -= amt;
            userObj.defaultWallet = String(data.walletAddress).trim();
            await userObj.save();
      ...
      case 'settings':
        const updateData = {};
        if (data.defaultWallet) updateData.defaultWallet = String(data.defaultWallet).trim();
        if (data.language) updateData.language = String(data.language).trim().toLowerCase();
        await User.findByIdAndUpdate(userId, updateData);
        break;
    }

    // إرجاع رد سريع جداً لمنع تعليق الواجهة
    return res.status(200).json({ success: true });
  } catch (err) {
    logger.error('Error in sendBeacon fallback handler: ' + err.message);
    return res.status(200).json({ success: true, error: err.message });
  }
});

// 1. مسار حفظ فوري للروابط
app.post('/api/links/save-instant', authMiddleware, async (req, res) => {
  try {
    const payload = parseBeaconPayload(req);
    const { title, targetUrl } = payload;
...
// 2. مسار حفظ فوري للحملات
app.post('/api/ads/save-instant', authMiddleware, async (req, res) => {
  try {
    const payload = parseBeaconPayload(req);
    const { title, targetUrl, totalBudget } = payload;
    const budget = Number(totalBudget);

    if (title && validUrl.isWebUri(targetUrl) && !isNaN(budget) && budget >= 5) {
      const userObj = await User.findOne({ _id: req.userId, availableBalance: { $gte: budget } });
      if (userObj) {
        userObj.availableBalance -= budget;
        await userObj.save();
...
// 3. مسار حفظ فوري للإيداع
app.post('/api/deposit/save-instant', authMiddleware, async (req, res) => {
  try {
    const payload = parseBeaconPayload(req);
    const { amount, network, txid } = payload;
    const numAmount = Number(amount);

    if (!isNaN(numAmount) && numAmount >= 1 && txid) {
      const existing = await Deposit.findOne({ txid: String(txid).trim() });
      if (!existing) {
        await Deposit.create({
          userId: req.userId,
          advertiserId: req.userId,
          advertiserTelegramId: req.user.telegramId,
          amount: numAmount,
          network: String(network || 'BEP20').toUpperCase(),
...
// 4. مسار حفظ فوري للسحب
app.post('/api/withdraw/save-instant', authMiddleware, async (req, res) => {
  try {
    const payload = parseBeaconPayload(req);
    const { amount, network, walletAddress } = payload;
    const numAmt = Number(amount);

    if (!isNaN(numAmt) && numAmt >= 30 && walletAddress) {
      const userObj = await User.findOne({ _id: req.userId, availableBalance: { $gte: numAmt } });
      if (userObj) {
        userObj.availableBalance -= numAmt;
        userObj.defaultWallet = String(walletAddress).trim();
        await userObj.save();
...
// 5. مسار حفظ فوري للمحفظة والإعدادات
app.post('/api/wallet/save-instant', authMiddleware, async (req, res) => {
  try {
    const payload = parseBeaconPayload(req);
    const { defaultWallet, language } = payload;
    const updateData = {};
