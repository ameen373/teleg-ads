/**
 * Ultra-Enterprise Server Architecture (Production Ready)
 * Telega.ads Platform & Link Shortener
 * Multi-Tenant Isolated Session System & Financial Security Core
 */
...
redis.on('error', (err) => {
  ...
  logger.error('⚠️ Redis Warning: ' + err.message);
});
redis.on('ready', () => {
...
const linkCreationLimiter = rateLimit({
  ...
  legacyHeaders: false,
  message: { success: false, message: 'تم تجاوز الحد اليومي لإنشاء الروابط' }
});
...
const validateTraffic = (req, res, next) => {
  ...
  if (botPattern.test(ua)) {
    return res.status(403).json({ success: false, message: 'تم رفض الزيارة الآلية (Bot Traffic Rejected)' });
  }
  next();
};
...
// =========================================================================
// --- Middleware: Auth Security Guard & Strict Isolation Layer ---
// =========================================================================
const authMiddleware = async (req, res, next) => {
  try {
    ...
    if (!user) {
      return res.status(401).json({ success: false, message: 'جلسة غير صالحة، يرجى إعادة تسجيل الدخول' });
    }
    ...
    res.status(401).json({ success: false, message: 'انتهت الجلسة، يرجى إعادة التسجيل' });
  }
};

const adminMiddleware = async (req, res, next) => {
  if (!req.user || String(req.user.telegramId).trim() !== CONFIG.ADMIN_ID) {
    return res.status(403).json({ success: false, message: 'غير مصرح لك بالوصول للوحة التحكم' });
  }
  next();
};
...
app.post('/api/auth/login', async (req, res, next) => {
  try {
    ...
    const tgId = telegramUser 
      ...
      : (process.env.NODE_ENV !== 'production' ? String(req.headers['x-demo-user-id'] || '') : null);
      
    const { referrerId } = req.body;

    if (!tgId) {
      return res.status(401).json({ success: false, message: 'بيانات الاعتماد الخاصة بتليجرام غير صالحة' });
    }
...
// =========================================================================
// --- User Profile & Comprehensive Data Endpoint ---
// =========================================================================
...
app.get('/api/user/data', authMiddleware, async (req, res, next) => {
  try {
    ...
    const [userProfile, rawLinks, withdraws, announcements, ads, deposits] = await Promise.all([
...
// =========================================================================
// --- Short Links Engine (Isolated per req.user.id) ---
// =========================================================================
...
const handleShortenLink = async (req, res) => {
  try {
    ...
    if (isPhishingOrMalicious(cleanUrl)) {
      return res.status(400).json({ success: false, message: 'الرابط ينتهك معايير الأمان' });
    }

    try {
      const domainCheck = new URL(cleanUrl).hostname;
      if (domainCheck.includes(CONFIG.APP_DOMAIN)) {
        return res.status(400).json({ success: false, message: 'لا يمكن اختصار روابط الموقع نفسه' });
      }
    } catch (e) {}
    ...
    return res.status(500).json({ success: false, message: 'حدث خطأ أثناء اختصار الرابط' });
  }
};
...
app.get(['/api/links', '/api/user/links'], authMiddleware, async (req, res, next) => {
  try {
    const rawLinks = await Link.find({ userId: req.user.id }).sort({ createdAt: -1 }).lean();
...
app.post('/api/links/toggle', authMiddleware, async (req, res, next) => {
  try {
    ...
    if (!link) {
      return res.status(404).json({ success: false, message: 'الرابط غير موجود أو لا تملك صلاحية التعديل عليه' });
    }
    ...
    res.json({ success: true, message: 'تم تغيير حالة الرابط بنجاح', data: { isActive: link.isActive } });
  } catch (err) {
    next(err);
  }
});
...
// =========================================================================
// --- Ad Campaigns Engine (Atomic Financial Operations) ---
// =========================================================================
...
app.post('/api/ads', authMiddleware, async (req, res, next) => {
  ...
  try {
    ...
    if (!updatedUser) {
      await session.abortTransaction();
      return res.status(400).json({ success: false, message: 'رصيدك المتاح غير كافي لإنشاء هذه الحملة' });
    }
    ...
    res.json({ success: true, message: 'تم إنشاء الحملة بنجاح', data: ad[0] });
  } catch (err) {
    await session.abortTransaction();
    next(err);
  } finally {
...
app.get(['/api/ads', '/api/user/ads'], authMiddleware, async (req, res, next) => {
  try {
...
app.post('/api/ads/toggle', authMiddleware, async (req, res, next) => {
  try {
    ...
    if (!ad) {
      return res.status(404).json({ success: false, message: 'الإعلان غير موجود أو لا تملك صلاحية تعديله' });
    }
    ...
    res.json({ success: true, message: 'تم تغيير حالة الحملة بنجاح', data: { status: ad.status } });
  } catch (err) {
    next(err);
  }
});
...
// =========================================================================
// --- Financial Wallet Engine (Deposit, Withdraw & Transactions) ---
// =========================================================================
...
app.post('/api/deposit', authMiddleware, async (req, res, next) => {
  try {
    ...
    if (isNaN(numAmount) || numAmount < 1) {
      return res.status(400).json({ success: false, message: 'الحد الأدنى للإيداع هو $1' });
    }
    ...
    if (existingDeposit) {
      return res.status(400).json({ success: false, message: 'تم تقديم رقم هذه المعاملة (TxID) من قبل' });
    }
    ...
    res.json({ success: true, message: 'تم تقديم طلب الإيداع بنجاح، وهو قيد المراجعة', data: deposit });
  } catch (err) {
    next(err);
  }
});
...
app.get('/api/user/deposits', authMiddleware, async (req, res, next) => {
  try {
    const deposits = await Deposit.find({ userId: req.user.id }).sort({ createdAt: -1 }).lean();
...
app.post('/api/withdraw', authMiddleware, async (req, res, next) => {
  ...
  try {
    ...
    if (isNaN(numAmt) || numAmt < 30) {
      await session.abortTransaction();
      return res.status(400).json({ success: false, message: 'الحد الأدنى للسحب هو $30' });
    }

    if (!['BEP20', 'TRC20', 'TON'].includes(cleanNetwork)) {
      await session.abortTransaction();
      return res.status(400).json({ success: false, message: 'يرجى تحديد الشبكة (BEP20, TRC20, TON)' });
    }
    ...
    const activePending = await Withdraw.findOne({ userId: req.user.id, status: 'pending' }).session(session);
    if (activePending) {
      await session.abortTransaction();
      return res.status(400).json({ success: false, message: 'لديك طلب سحب قيد الانتظار حالياً' });
    }
    ...
    if (!updatedUser) {
      await session.abortTransaction();
      return res.status(400).json({ success: false, message: 'رصيدك المتاح لا يكفي لإتمام عملية السحب' });
    }
    ...
    res.json({ success: true, message: 'تم تقديم طلب السحب بنجاح', data: withdrawRequest[0] });
  } catch (err) {
    await session.abortTransaction();
    next(err);
  } finally {
...
app.get('/api/user/withdraws', authMiddleware, async (req, res, next) => {
  try {
    const withdraws = await Withdraw.find({ userId: req.user.id }).sort({ createdAt: -1 }).lean();
...
app.get('/api/user/transactions', authMiddleware, async (req, res, next) => {
  try {
    const userId = req.user.id;
    const [deposits, withdraws, holds] = await Promise.all([
      Deposit.find({ userId: userId }).sort({ createdAt: -1 }).lean(),
      Withdraw.find({ userId: userId }).sort({ createdAt: -1 }).lean(),
      EarningsHold.find({ userId: userId }).sort({ createdAt: -1 }).lean()
    ]);

    const transactions = [
      ...deposits.map(d => ({ ...d, type: 'deposit' })),
      ...withdraws.map(w => ({ ...w, type: 'withdraw' })),
      ...holds.map(h => ({ ...h, type: 'earning_hold' }))
...
// =========================================================================
// --- Traffic Redirection & Impression Calculation Engine ---
// =========================================================================
...
app.post('/api/init-click', validateTraffic, async (req, res, next) => {
  try {
    ...
    if (!cleanCode) return res.status(400).json({ success: false, message: 'كود الرابط مطلوب' });
...
app.post('/api/impression', validateTraffic, clickLimiter, async (req, res, next) => {
  ...
  try {
    ...
    if (!sessionId || !bridgeToken) {
      ...
      return res.status(400).json({ success: false, message: 'رمز حماية الجلسة مفقود' });
    }

    const cachedToken = await safeRedisGet(`bridge:token:${sessionId}`);
    if (cachedToken && cachedToken !== bridgeToken) {
      await sessionDb.abortTransaction();
      return res.status(403).json({ success: false, message: 'تم اكتشاف محاولة تخطي غير مشروعة' });
    }
    ...
    if (isDuplicate || dailyIpClicks > 20) {
      ...
      return res.json({ success: true, data: { targetUrl: link.targetUrl, counted: false } });
    }
    ...
    await sessionDb.commitTransaction();
    res.json({ success: true, data: { targetUrl: link.targetUrl, counted: true } });
  } catch (err) {
    await sessionDb.abortTransaction();
    next(err);
  } finally {
...
app.get('/api/admin/dashboard-data', authMiddleware, adminMiddleware, async (req, res, next) => {
  try {
    ...
    res.json({ 
      success: true, 
      data: { withdraws, deposits, users, stats: { ...(stats[0] || {}), totalAds } } 
    });
  } catch (err) {
    next(err);
  }
...
app.post('/api/admin/deposit/action', authMiddleware, adminMiddleware, async (req, res, next) => {
  ...
  if (!mongoose.Types.ObjectId.isValid(depositId)) {
    return res.status(400).json({ success: false, message: 'معرف الإيداع غير صالح' });
  }
  ...
  try {
    ...
    if (!deposit || deposit.status !== 'pending') {
      await session.abortTransaction();
      return res.status(400).json({ success: false, message: 'طلب الإيداع غير موجود أو تم معالجته سابقاً' });
    }
    ...
    await session.commitTransaction();
    res.json({ success: true, message: 'تم تحديث حالة الإيداع بنجاح', data: deposit });
  } catch (err) {
    await session.abortTransaction();
    next(err);
  } finally {
...
app.post('/api/admin/withdraw/action', authMiddleware, adminMiddleware, async (req, res, next) => {
  ...
  if (!mongoose.Types.ObjectId.isValid(withdrawId)) {
    return res.status(400).json({ success: false, message: 'معرف السحب غير صالح' });
  }
  ...
  try {
    ...
    if (!withdraw || withdraw.status !== 'pending') {
      await session.abortTransaction();
      return res.status(400).json({ success: false, message: 'طلب السحب غير موجود أو تم معالجته سابقاً' });
    }
    ...
    await session.commitTransaction();
    res.json({ success: true, message: 'تم تحديث حالة السحب بنجاح', data: withdraw });
  } catch (err) {
    await session.abortTransaction();
    next(err);
  } finally {
...
app.post('/api/admin/distribute-revenue', authMiddleware, adminMiddleware, async (req, res, next) => {
  ...
  try {
    ...
    if (isNaN(revenue) || revenue <= 0) {
      await session.abortTransaction();
      return res.status(400).json({ success: false, message: 'مبلغ الإيرادات غير صالح' });
    }
    ...
    if (totalImp === 0) {
      await session.abortTransaction();
      return res.status(400).json({ success: false, message: 'لا توجد مشاهدات مؤكدة لتوزيع الأرباح' });
    }
...
app.post('/api/admin/user/toggle-ban', authMiddleware, adminMiddleware, async (req, res, next) => {
  ...
  try {
    ...
    res.json({ success: true, message: 'تم تحديث حالة حظر المستخدم بنجاح', data: { isBanned: user.isBanned } });
  } catch (err) {
    next(err);
  }
});
...
// =========================================================================
// --- Automated Cron Settlement Engine ---
// =========================================================================
