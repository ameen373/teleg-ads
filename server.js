/**
 * Ultra-Enterprise Server Architecture (V7 Pro Max - Absolute Security, High-Performance & Dynamic Fallback Core)
 * Telegram Link Shortener & Mini App Engine (Telega.ads)
 * Complete Isolated Session System, Financial Security & Fraud Prevention Engine
 */
...
// --- Force UTF-8 JSON Response Headers & Privacy Guard ---
...
// ==================================================
// --- In-Memory Fast Cache Engine (Redis Fallback) ---
// ==================================================
class LocalCacheEngine {
  ...
  incr(key, ttlMs = 86400000) {
    const current = Number(this.get(key) || 0) + 1;
    this.set(key, current, ttlMs);
    return current;
  }
...
const localCache = new LocalCacheEngine();

// --- Redis Client Dynamic Handling ---
...
redis.connect().catch(err => {
  logger.warn('⚠️ Redis connection skipped/deferred: ' + err.message);
});
...
redis.on('ready', () => {
  ...
  console.log('✅ Enterprise Redis Client Active');
});

async function safeRedisGet(key) {
  if (redisIsConnected) {
    try { return await redis.get(key); } catch (e) { redisIsConnected = false; }
  }
...
async function safeRedisSet(key, value, mode, duration) {
  if (redisIsConnected) {
    try {
      ...
      return;
    } catch (e) { redisIsConnected = false; }
  }
  const ttlMs = (mode === 'EX' && duration) ? duration * 1000 : null;
  localCache.set(key, value, ttlMs);
}

async function safeRedisDel(key) {
  if (redisIsConnected) {
    try { await redis.del(key); } catch (e) { redisIsConnected = false; }
  }
  localCache.del(key);
}

async function safeRedisIncr(key, expireSec = 86400) {
  if (redisIsConnected) {
    try {
  ...
  return localCache.incr(key, expireSec * 1000);
}
...
}).then(() => console.log('✅ Enterprise Mongo Cluster Connected'))
  .catch(err => {
...
async function sendTelegramNotification(telegramId, message) {
  ...
  try {
    ...
    logger.error(`⚠️ Telegram Dispatch Error [ID: ${telegramId}]: ${err.message}`);
  }
}

// --- Cryptographic Telegram Authenticator (Strict HMAC Verification) ---
...
// --- Async Wrapper for Clean Express Routes ---
const asyncHandler = (fn) => (req, res, next) => {
  Promise.resolve(fn(req, res, next)).catch(next);
};

// =========================================================================
// --- Middleware للتحقق من هوية المستخدم وحماية الجلسات ---
// =========================================================================
...
app.post('/api/init-click', validateTraffic, asyncHandler(async (req, res) => {
  ...
  if (linkData) {
    const parsed = typeof linkData === 'string' ? JSON.parse(linkData) : linkData;
    linkId = parsed.id;
...
app.post('/api/impression', validateTraffic, clickLimiter, asyncHandler(async (req, res) => {
  ...
  try {
    ...
    const dailyIpClicks = await safeRedisIncr(dailyIpClickKey, 86400);
...
app.post('/api/links/shorten', authMiddleware, linkCreationLimiter, asyncHandler(handleShortenLink));
app.post('/api/links', authMiddleware, linkCreationLimiter, asyncHandler(handleShortenLink));
...
app.post('/api/admin/distribute-revenue', authMiddleware, adminMiddleware, asyncHandler(async (req, res) => {
  const session = await mongoose.startSession();
  session.startTransaction();
  try {
...
// --- Automated Settlement Cron Task ---
cron.schedule('0 0 * * *', async () => {
  try {
    ...
    logger.error('❌ Error executing Settlement Cron: ' + err.message);
  }
});
...
// --- Graceful Server Shutdown & Crash Guard ---
...
const server = app.listen(PORT, () => console.log(`🚀 Enterprise Server V7 Pro Max Active on Port ${PORT}`));
