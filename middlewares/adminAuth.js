...
const adminAuth = (req, res, next) => {
  // التحقق أولاً من وجود كائن المستخدم (كإجراء أمني إضافي)
  if (!req.user) {
  ...
  // تقسيم النص بواسطة الفاصلة، إزالة المسافات، تحويل كل عنصر لرقم، وتجاهل القيم غير الصالحة
  ...
  const isIdAdmin = adminIdsArray.includes(req.user.telegramId);

  if (isRoleAdmin || isIdAdmin) {
    // إذا تحقق أي من الشرطين، يتم السماح بالمرور للمسار التالي
    return next();
  }

  // في حال عدم تحقق الشروط، يتم إرجاع استجابة 403 (Forbidden)
