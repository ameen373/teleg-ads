...
/**
 * Isolated User Dashboard Data Endpoint
 */
 ...
 */
router.get('/api/user/referrals', resolveUserId, async (req, res, next) => {
  try {
    await connectDB();
    const targetUserId = req.userId;
 ...
 */
router.post('/api/withdraw', resolveUserId, async (req, res, next) => {
  await connectDB();
  ...
  try {
    ...
    res.json({ success: true, withdraw: withdraw[0], message: 'تم إرسال طلب السحب بنجاح وسياتم معالجته قريباً' });
  } catch (err) {
    await session.abortTransaction();
    next(err);
  } finally {
...
/**
 * User Financial Transactions History
 */
router.get('/api/user/transactions', resolveUserId, async (req, res, next) => {
  try {
    await connectDB();
    const targetUserId = req.userId;
 ...
 */
router.post('/api/user/settings', resolveUserId, async (req, res, next) => {
  try {
    await connectDB();
